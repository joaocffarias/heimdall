  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={`card p-6 space-y-5 animate-slide-up ${step === 1 ? 'block' : 'hidden'}`}>
          ...
          <div className="flex justify-end">
            <button type="button" onClick={async () => {
              const isValid = await trigger(['visitorName', 'visitorDoc', 'visitorType', 'destination', 'purpose']);
        ...
        {/* ─── Step 2: Materiais ──────────────────────────────────── */}
        <div className={`space-y-4 animate-slide-up ${step === 2 ? 'block' : 'hidden'}`}>
                ...
                <div key={field.id} className="bg-navy-800 rounded-xl border border-navy-500 p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      ...
                      <input {...register(`materials.${idx}.name` as const, { required: true })}
                        className="input" placeholder="Ex: Notebook Dell, Chave de fenda..." />
                    </div>

                    <div>
                      <label className="input-label">Categoria</label>
                      <select {...register(`materials.${idx}.category` as const)} className="input">
                        {CATEGORIES.map((c) => (
                    ...
                    <div>
                      ...
                      <input {...register(`materials.${idx}.quantity` as const, { valueAsNumber: true, min: 1 })}
                        type="number" min={1} className="input" />
                    </div>

                    <div>
                      <label className="input-label">Nº de Série / Patrimônio</label>
                      <input {...register(`materials.${idx}.serialNumber` as const)}
                        className="input" placeholder="Opcional" />
                    </div>

                    <div>
                      <label className="input-label">Marca / Modelo</label>
                      <input {...register(`materials.${idx}.brand` as const)}
                        className="input" placeholder="Opcional" />
                    </div>
                  </div>
        ...
        {/* ─── Step 3: Responsável ────────────────────────────────── */}
        <div className={`card p-6 space-y-5 animate-slide-up ${step === 3 ? 'block' : 'hidden'}`}>
          <h2 className="font-semibold text-slate-100 flex items-center gap-2">
